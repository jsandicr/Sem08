...
class UpdateLugarFragment : Fragment() {
    ...
    private fun updateLugar(){
        ...
        if(nombre.isNotEmpty()){
            ...
            Toast.makeText(requireContext(),getString(R.string.msg_UpdateLugar_correcto), Toast.LENGTH_LONG).show()
            findNavController().navigate(R.id.action_updateLugarFragment_to_nav_home)
        }else{
            Toast.makeText(requireContext(),getString(R.string.msg_AddLugar_sinNombre), Toast.LENGTH_LONG).show()
        }
    }

    private fun deleteLugar(){
        homeViewModel.eliminarLugar(args.lugarArg.id)
        Toast.makeText(requireContext(),getString(R.string.msg_DeleteLugar_correcto), Toast.LENGTH_LONG).show()
        findNavController().navigate(R.id.action_updateLugarFragment_to_nav_home)
    }
}